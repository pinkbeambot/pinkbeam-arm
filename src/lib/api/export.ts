...
export function toCsv<T extends Record<string, unknown>>(
  data: T[],
  options?: { columns?: string[]; includeHeaders?: boolean; }
): string {
  ...
  const rows: string[] = [];
  if (includeHeaders) rows.push(columns.join(','));

  for (const row of data) {
    const values = columns.map(col => {
      const value = row[col];
      return typeof value === 'object' && value !== null 
        ? escapeCell(JSON.stringify(value)) 
        : escapeCell(value);
    });
...
export function formatForExport<T extends Record<string, unknown>>(
  ...
  switch (format) {
    case 'csv': return toCsv(data, options);
