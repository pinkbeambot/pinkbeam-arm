...
export function useDecisionsRealtime(options: UseDecisionsOptions = {}) {
  ...
  // Use ref to track current options for realtime updates
  ...
  }, [agentId, status, category, dateFrom, dateTo, confidenceMin, search, page, limit]);
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload: RealtimeChangePayload<Decision>) => {
          // Only refetch if we're on the first page and no filters applied
          // Otherwise, the user might lose their position
          ...
          if (
            ...
            // Update the specific decision in the list
...
export function useUpdateDecision() {
  ...
  const updateDecision = useCallback(async (
    decisionId: string,
    updateData: { status?: DecisionStatus; outcome?: Record<string, unknown>; executed_action?: Record<string, unknown> }
  ) => {
    setLoading(true);
    setError(null);
